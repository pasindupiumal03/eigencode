    ...
    <div className="w-full">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-start">
        <div className="bg-zinc-900 backdrop-blur-lg p-6 rounded-lg transition-all duration-300 animated-border">
          ...
          >
            <input className="hidden" accept="image/*" type="file" ref={fileInputRef} onChange={handleImageUpload} />
            {imageSrc ? (
              ...
              <>
                ...
                <h3 className="font-bold mb-2 text-center text-blue-400">Upload Image</h3>
                <p className="text-gray-400 text-center text-sm">Click or drag an image here</p>
              </>
            )}
          </div>
        </div>
        ...
        <div className="bg-zinc-900 backdrop-blur-lg p-6 rounded-lg transition-all duration-300 animated-border">
          <div className="border-2 border-blue-500/30 rounded-lg p-8 flex flex-col items-center justify-center min-h-[300px] relative">
            {finalImage ? (
              <img src={finalImage || "/placeholder.svg"} alt="Transformed" className="max-w-full max-h-full rounded" />
            ) : (
              <>
                <div className="text-blue-400/50 mb-4 text-6xl">⟁</div>
                <h3 className="font-bold mb-2 text-center text-blue-400/50">Transformed Result</h3>
                <p className="text-gray-400 text-center text-sm">Apply filter to see the result</p>
              </>
            )}
          </div>
        </div>
          ...
          {Object.keys(previewFilters).map((key) =>
            key !== "matrixEffect" ? (
              <div key={key} className="flex flex-col">
                <label className="text-sm capitalize text-white mb-1">{key}</label>
                <input
                  type="range"
                  min="0"
                  max="200"
                  value={previewFilters[key as keyof typeof previewFilters] as number}
                  onChange={(e) =>
                    setPreviewFilters({
                      ...previewFilters,
